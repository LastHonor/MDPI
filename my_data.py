            ...
            if data_name == "positions":
                ...
                if flex_engine:
                    ...
                    if "ok_obj_id" in static_data_info:
                        ...
                        if "bad_lamp" in static_data_info and obj_positions.shape[0] != \
...
def remove_large_obstacles(phases_dict):
    if phases_dict["n_particles"] > 3000:
        ...
        critical_objects = [b'cloth_square', b'buddah', b'bowl', b'cone', b'cube', b'cylinder', b'dumbbell',
                            b'octahedron', b'pentagon', b'pipe', b'platonic', b'pyramid', b'sphere', b'torus',
                            b'triangular_prism']
        import copy
...
def subsample_particles_on_all_objects(phases_dict, limit=3000):
    n_objects = phases_dict["n_objects"]
    obj_names = phases_dict["instance"]
    obj_subsample_idx = []
    ...
    obj_points = phases_dict["obj_points"]
    # print([obj_pts.shape[0] for obj_pts in obj_points])
    old_n_particles = phases_dict["n_particles"]

    shrink_rate = limit / old_n_particles
    ...
    for obj_id in range(n_objects):
        obj_npts = phases_dict["instance_idx"][obj_id + 1] - phases_dict["instance_idx"][obj_id]
        # print(obj_id, obj_npts, phases_dict["instance"][obj_id])
        if obj_names[obj_id] != b'cloth_square':
            selected_idxs = np.random.choice(obj_npts, int(shrink_rate * obj_npts), replace=False)
            obj_points[obj_id] = obj_points[obj_id][selected_idxs]
            obj_subsample_idx.append(selected_idxs)
            # phases_dict["clusters"][0][0][0] -> phases_dict["clusters"][obj_id][nodeid_in_a_root_set][0]
            is_subsample = True
...
def make_hierarchy(env, attr, positions, velocities, idx, st, ed, phases_dict, count_nodes, clusters, verbose=0,
                   var=False):
    order = 2
...
def prepare_input(data, stat, args, phases_dict, verbose=0, var=False):
    ...
    if args.env == "TDWdominoes":
        # attributes:
        # [1, 0, 0]: water particle   ? 注释反了？
        # [0, 1, 0]: rigid particle   ? 注释反了？
    ...
    if len(rels) > 0:
        ...
        if rels.shape[0] > 0:
            ...
            Rr_idxs.append(torch.LongTensor(np.array([rels[:, 0], np.arange(rels.shape[0])])))
            Rs_idxs.append(torch.LongTensor(np.array([rels[:, 1], np.arange(rels.shape[0])])))
        ...
        for i in range(len(instance_idx) - 1):
            ...
            if n_root_level > 0:
                ...
                for j in range(len(rels)):
                    ...
                    Rr_idxs.append(torch.LongTensor(np.array([rels[j][:, 0], np.arange(rels[j].shape[0])])))
                    Rs_idxs.append(torch.LongTensor(np.array([rels[j][:, 1], np.arange(rels[j].shape[0])])))
...
class PhysicsFleXDataset(Dataset):
    ...
    def __init__(self, args, phase, phases_dict, verbose):
        ...
        self.data_dir = [os.path.join(dataf, phase).replace("\\", "/") for dataf in self.args.dataf]

        if self.args.statf:
            self.stat_path = os.path.join(self.args.data_root, self.args.statf).replace("\\", "/")
        else:
            self.stat_path = None
        ...
        if self.args.n_rollout is None:
            self.all_trials = []
            self.n_rollout = 0
            ...
            for ddir in self.data_dir:
                ...
                self.all_trials += [os.path.join(ddir_root, trial_name).replace("\\", "/") for trial_name in
                                    trial_names]
                self.n_rollout += n_trials
                # self.all_trials = self.all_trials[:len(self.all_trials) // 10]
                # self.n_rollout = self.n_rollout // 10

            if phase == "train":
                self.mean_time_step = 1  # int(13499 / self.n_rollout) + 1
            else:
        ...
        self.temp_data_dir = './myTempDataForDataLoader'
    ...
    def augment_worldcoord(self, list_of_data):
        ...
        for data in list_of_data:
            ...
            # (trimesh.Scene(pcd) + axis).show()
            ## plot the two
            transformed_positions = np.matmul(RT, np.concatenate([positions, np.ones((positions.shape[0], 1))],
                                                                 axis=1).T).T[:, :3]
            transformed_velocities = np.matmul(R, np.concatenate([velocities, np.ones((velocities.shape[0], 1))],
                                                                 axis=1).T).T[:, :3]
    ...
    def __getitem__(self, idx):
        ...
        if self.args.env == "TDWdominoes":
            ...
            #  点最多 pilot_it2_drop_sizes_box_2_dis_2_occ_0000
            #  时间步最长 (drop pilot_it2_drop_all_bowls_box_0005)
            #  (link pilot_linking_nl1-5_aNone_bCube_occ1_dis1_tdwroom_0001)
            # trial_dir = trial_dir.split("/")[:-1] + ['pilot_it2_drop_sizes_box_2_dis_2_occ_0000']
            #                                          # if self.args.dataf[0].split("/")[-1] == 'Drop'
            #                                          # else 'pilot_linking_nl1-5_aNone_bCube_occ1_dis1_tdwroom_0001']
            # trial_dir = "/".join(trial_dir)
            # print("dummy data")
            ...
            pkl_path = os.path.join(trial_dir, 'phases_dict.pkl').replace("\\", "/")
            ...
            #
            # is_subsample = subsample_particles_on_all_objects(phases_dict, limit=3000)

            # because we want to sample also idx_timestep + 1
            time_step = phases_dict["time_step"] - self.training_fpt
            # if self.phase == 'train':
            #     idx_timestep_min = np.random.randint(self.start_timestep, time_step)
            # else:
            idx_timestep_min = self.start_timestep
        ...
        sample_path = self.temp_data_dir + f'/{trial_dir.split("/")[-1]}.pth'
        # if not os.path.exists(sample_path) or 'drape' in trial_dir.lower():
        ...
        if True:
            ...
            # if self.args.subsample_particles:
            is_subsample = subsample_particles_on_large_objects(phases_dict, limit=2500 if 'drape' in trial_dir.lower() else 3000)

            # is_subsample = subsample_particles_on_all_objects(phases_dict, limit=1000)

            attr_list, state_list, relations_list, n_particles_list, \
                n_shapes_list, instance_idx_list, label_list, phases_dict_list = [], [], [], [], [], [], [], []

            # if idx_timestep_min + 1 < phases_dict["time_step"] - int(self.training_fpt):
            #     end_idx = np.random.randint(idx_timestep_min + 1, phases_dict["time_step"] - int(self.training_fpt))
            # else:
            end_idx = phases_dict["time_step"] - 1 - int(self.training_fpt)
            for idx_timestep in range(idx_timestep_min, end_idx, int(self.training_fpt)):

                data_path = os.path.join(data_dir, trial_fullname, str(idx_timestep) + '.h5').replace("\\", "/")
                data_nxt_path = os.path.join(data_dir, trial_fullname,
                                             str(int(idx_timestep + self.training_fpt)) + '.h5').replace("\\", "/")
                current_oldest = 0
                ...
                data_prev_path = os.path.join(data_dir, trial_fullname,
                                              str(max(0, int(idx_timestep - self.training_fpt))) + '.h5').replace("\\",
                                                                                                                  "/")
                ...
                # adding history
                data_hiss = []
                path = os.path.join(data_dir, trial_fullname,
                                    str(max(0, int(idx_timestep - (1) * self.training_fpt))) + '.h5').replace("\\", "/")
                data_cur = load_data_dominoes(self.data_names, path, phases_dict)
                for i in range(self.args.n_his):
                    path = os.path.join(data_dir, trial_fullname,
                                        str(max(0,
                                                int(idx_timestep - (i + 1 + 1) * self.training_fpt))) + '.h5').replace(
                        "\\",
                        "/")
                    if self.args.env == "TDWdominoes":
                ...
                if self.augment_coord or self.training_fpt > 1:
                    ...
                    path = os.path.join(data_dir, trial_fullname,
                                        str(max(0,
                                                int(idx_timestep - current_oldest - self.training_fpt))) + '.h5').replace(
                        "\\", "/")
                ...
                if self.args.noise_std > 0:
                    ...
                    vel_sequence_noise = np.random.normal(0, self.args.noise_std / (n_velsteps ** 0.5),
                                                          size=(n_velsteps, vel_dim))
                ...
                n_shapes_list.append(n_shapes)
                instance_idx_list.append(instance_idx)
                label_list.append(label)

                # break
            # attr_list = attr_list + attr_list[-1:] * (205 - len(attr_list))
            # state_list = state_list + state_list[-1:] * (205 - len(state_list))
            # relations_list = relations_list + relations_list[-1:] * (205 - len(relations_list))
            # n_particles_list = n_particles_list + n_particles_list[-1:] * (205 - len(n_particles_list))
            # n_shapes_list = n_shapes_list + n_shapes_list[-1:] * (205 - len(n_shapes_list))
            # instance_idx_list = instance_idx_list + instance_idx_list[-1:] * (205 - len(instance_idx_list))
            # label_list = label_list + label_list[-1:] * (205 - len(label_list))

            # save_list = [attr_list, state_list, relations_list, n_particles_list, n_shapes_list, instance_idx_list,
            #              label_list, phases_dict]
            # if 'drape' not in trial_dir.lower():
            #     torch.save(save_list, sample_path)

        else:
            save_list = torch.load(sample_path)
            attr_list, state_list, relations_list, n_particles_list, n_shapes_list, instance_idx_list, \
                label_list, phases_dict = save_list

        return attr_list, state_list, relations_list, n_particles_list, \
            n_shapes_list, instance_idx_list, label_list, phases_dict
