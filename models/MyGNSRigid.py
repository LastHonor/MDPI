...
class MyGNSRigid(nn.Module):
    def __init__(self, args, stat, phases_dict, residual=False, use_gpu=False):
        ...
        self.lstm_cell = LSTMCell(nf_effect)
    ...
    def forward(self, attr, state, Rr, Rs, Ra, Rr_idxs, n_particles, node_r_idx, node_s_idx, pstep, rels_types,
                instance_idx, phases_dict, particles_c, particles_h, verbose=1):
        """
        ...
        state_r_s_rel = torch.cat([state_r_rel[:, :3] - state_s_rel[:, :3],
                                   torch.norm(state_r_rel[:, :3] - state_s_rel[:, :3], p=2, keepdim=True, dim=1)],
                                  dim=1)
        ...
        return normalized_velocities, particles_c, particles_h
