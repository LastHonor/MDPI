...
class LSTMCell(nn.Module):
    ...
    def forward(self, c, h, x):
        ...
        if c is None:
            c = torch.zeros_like(x, device='cuda' if torch.cuda.is_available() else 'cpu')
        if h is None:
            h = torch.zeros_like(x, device='cuda' if torch.cuda.is_available() else 'cpu')

        x = torch.cat([x, h], dim=-1)

        # f = self.forget_gate(x)
        # i1 = self.input_gate1(x)
        i2 = self.input_gate2(x)
        # i = i1 * i2
...
class MyModel0(nn.Module):
    def __init__(self, args, stat, phases_dict, residual=False, use_gpu=False):
        ...
        for i in range(args.n_stages):
            self.lstm_cell_list.append(LSTMCell(nf_effect))
    ...
    def forward(self, attr, state, Rr, Rs, Ra, Rr_idxs, n_particles, node_r_idx, node_s_idx, pstep, rels_types,
                instance_idx, phases_dict, particles_c, particles_h, verbose=0):
        """
        ...
        if particles_c is None:
            particles_c = torch.zeros_like(particle_effect, device='cuda' if torch.cuda.is_available() else 'cpu')
        if particles_h is None:
            particles_h = torch.zeros_like(particle_effect, device='cuda' if torch.cuda.is_available() else 'cpu')
        ...
        for stage_id in range(self.n_stages):
            ...
            for i in range(psteps):
                ...
                effect_p_r_previous = particles_h[node_r_idxs]

                receiver_effect_previous = particles_h[Rrp_global_idxs]
                sender_effect_previous = particles_h[Rsp_global_idxs]

                if psteps == 0:
                    _, effect_p_r = self.lstm_cell_list[stage_id](particles_c, effect_p_r_previous, effect_p_r)
                    _, receiver_effect = self.lstm_cell_list[stage_id](particles_c, receiver_effect_previous,
                                                                       receiver_effect)
                    _, sender_effect = self.lstm_cell_list[stage_id](particles_c, sender_effect_previous, sender_effect)
            ...
            # particles_c, particles_h = self.lstm_cell_list[stage_id](particles_c, particles_h, particle_effect)
            # particle_effect[node_r_idxs] = particles_h[node_r_idxs]

        # LSTM Cell
        # particles_c, particles_h = self.lstm_cell(particles_c, particles_h, particle_effect)
        #
        # particle_effect = particles_h
        particles_h = particles_effect
        ...
        return pred, particles_c, particles_h
