...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, d_model, max_len=500):
        ...
        pe = torch.zeros(1, max_len, d_model, requires_grad=True, device='cuda')
        # position = torch.arange(0, max_len, dtype=torch.float).view(1, -1, 1)
        # div_term = torch.exp(torch.arange(0, d_model, 2).float() * (-np.log(10000.0) / d_model)).view(1, 1, -1)
...
class TransformerEncoder(nn.Module):
    def __init__(self, input_size, num_layers, heads, dropout, use_gpu, small_batch=256):
        ...
        self.li_encoder = nn.Linear(input_size, input_size)
        self.li_decoder = nn.Linear(input_size, input_size)
        self.pe = PositionalEncoding(d_model=input_size)
        encoder_layer = nn.TransformerEncoderLayer(input_size, dim_feedforward=input_size, nhead=heads, batch_first=True, dropout=dropout, norm_first=True)
        self.encoder = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)
        self.use_gpu = use_gpu
        self.small_batch = small_batch

    def forward(self, x_his, x):
        """
        Args:
            x: [n_particles, input_size]
        ...
        Returns:
            [n_particles, output_size]
        """
        x = torch.unsqueeze(x, 1)  # [n_particles, 1, input_size]
        ...
        if x_his is None:
            ...
            # if self.use_gpu:
            #     x_his = x_his.cuda()
            x = torch.cat([x_his[:, -2:, :], x], 1)
            # x = torch.cat([x_his, x], 1)

        x = self.li_encoder(x)
        x = self.pe(x)
        # n = x.size(0)
        # o = []
        # for i in range(0, n, self.small_batch):
        #     o.append(self.encoder(x[i: i + self.small_batch])[:, -1:, :])
        # o = torch.cat(o, dim=0)
        # x_his = x
        o = self.encoder(x)[:, -1:, :]
        ...
        if x_his is None:
            ...
            x_his = torch.cat([x_his, o], 1)
        return o[:, -1, :], x_his.detach()
        # return o[:, -1, :], o
...
class MyModel3(nn.Module):
    ...
    def forward(self, attr, state, Rr, Rs, Ra, Rr_idxs, n_particles, node_r_idx, node_s_idx, pstep, rels_types,
                instance_idx, phases_dict, particle_effects, verbose=0):
        """
        ...
        particle_effect, particle_effects = \
            self.attention(particle_effects, particle_effect)
        # if particles_effects is None:
        #     particles_effects = torch.zeros_like(particle_effect, device='cuda').unsqueeze(1)
        # particle_effects = torch.cat([particles_effects, torch.unsqueeze(particle_effect, 1)], 1)

        # particle_effect = self.lstm_h_decoder(torch.cat([particle_h, particle_effect], dim=1))
        # particle_effect = particle_effect.squeeze()
