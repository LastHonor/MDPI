...
if args.model_name == "DPINet2":
    ...
    to do: add relationship type
    """
    model = MyModel1(args, stat, phases_dict, residual=True, use_gpu=use_gpu)
...
print("Loading network from %s for prediction of %s" % (model_file, args.dataf.split("\\")[-1]))
...
# result_dict = {
#         'trail_name': trial_name.split("\\")[-1],
#         "time_length": args.time_step,
#         "real_label": label_gt,
#         "predict_label": pred_is_positive_trial,
#         "loss_list": loss_list
#     }
result_dict_list = []  # [{trial_name: "...", real_label: True, predict_label: False, loss_list_in_time_steps: [0,-1]}]
if not os.path.exists("dummyLog/PredictionDetails"):
    os.mkdir("dummyLog/PredictionDetails")
detail_csv_record = "dummyLog/PredictionDetails" + f"/{modelf}-{dataf}{args.epoch if args.epoch >= 0 else ''}.csv"
with open(detail_csv_record, "w") as f:
    f.write("TrailName,TimeLength,RealLabel,PredictLabel,Acc,LossList\n")
...
for trial_id, trial_cxt in enumerate(gt_labels):
    ...
    if args.test_training_data_processing:
        ...
        if not is_bad_chair and not is_remove_obstacles and not is_subsample:
            ...
            pass
            # print("is_bad_chair", is_bad_chair, "is_remove_obstacles", is_remove_obstacles, "is_subsample",
            #       is_subsample)
    ...
    # model rollout
    start_timestep = 45  # 15
    step = start_timestep - args.training_fpt
    start_id = 15  # 5
    ...
    for current_fid in range(total_nframes - start_id):
        ...
        with torch.set_grad_enabled(False):
            ...
            #     instance_idx, phases_dict, c, h, args.verbose_model)
            ...
            # if args.debug:
        if step + args.training_fpt in timesteps:
            data_current = load_data_dominoes(data_names, data_path, phases_dict)
            data_nxt_path = os.path.join(trial_name, f'{int(step + args.training_fpt)}.h5')
            data_nxt = load_data_dominoes(data_names, data_nxt_path, phases_dict)
            _, data_nxt = recalculate_velocities([data_current, data_nxt], dt, data_names)
    ...
    print(f"\rTest: {trial_id}/{total_trial_num} | "
          f"ACC: {np.mean(accs) * 100: .02f}% | "
          f"Loss: {total_loss_sum / (total_loss_count + 1e-7): .06f}", end='')
    ...
    with open(detail_csv_record, "a") as f:
        f.write(f"{result_dict['trail_name']},{args.time_step},{label_gt},"
                f"{pred_is_positive_trial},{1 if label_gt == pred_is_positive_trial else 0}")
    ...
    #     break
with open(detail_csv_record, "a") as f:
    f.write(f" , , , ,{np.mean(accs)}")

print()
all_acc = np.mean(accs).item()
if not os.path.exists('dummyLog/acc/'):
    os.mkdir('dummyLog/acc/')
file_path = 'dummyLog/acc/' + f'{modelf}-{dataf}{args.epoch if args.epoch >= 0 else ""}.txt'
with open(file_path, 'w') as f:
    f.write(str(all_acc) + "\n" + str(total_loss_sum / (total_loss_count + 1e-7)))

np.save('dummyLog/PredictionDetails/' + f'{modelf}-{dataf}{args.epoch if args.epoch >= 0 else ""}.npy', result_dict_list, allow_pickle=True)
