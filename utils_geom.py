...
def mesh_to_particles(mesh_filename, spacing=0.2, visualization=False, remove_outofbbox_pts=True):
    ...
    assert not os.path.isfile(
        output_binvox_filename), f"binvox file {output_binvox_filename} exists, please delete it first"
    ...
    os.system(
        f'binvox -aw -dc -d {maxDim_spaced} -bb {meshLower_spaced[0]} {meshLower_spaced[1]} {meshLower_spaced[2]} {meshUpper_spaced[0]} {meshUpper_spaced[1]} {meshUpper_spaced[2]} -t binvox {mesh_filename}')
    # print(meshLower_spaced, meshUpper_spaced)
    # os.system(f'binvox -aw -dc -d {60} -t binvox {mesh_filename}')
    ...
    points = np.expand_dims(meshLower_spaced, 0) + np.stack(
        [(x + 0.5) * adjusted_spacing, (y + 0.5) * adjusted_spacing, (z + 0.5) * adjusted_spacing], axis=1)
    ...
    if remove_outofbbox_pts:
        ...
        lower_bound = bbox[0, :]
        upper_bound = bbox[1, :]
        # lower_bound = meshLower_spaced[:]
        # upper_bound = meshUpper_spaced[:]
